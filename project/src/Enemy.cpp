...
void Enemy::render(const Game &game) const {
    ...
    if (game.renderShadows) {
        glPushMatrix();
        {
            ...
            if (position.X() > 1) {
              glScalef(scale * (1 - (position.X() / 40.f)),
                       scale * (1 - (position.X() / 40.f)),
                       scale * (1 - (position.X() / 40.f)));
            }
            else {
            ...
            glScalef(0.f, 1.01f, 1.01f); // Appiattisco sulla Y, ingrandisco dell' 1% sulla X e la Z
...
void Enemy::doStep(const StarDestroyer &sD) {
    /* CALCOLA NUOVA POSIZIONE */
    {
        const Point3 sDCoord = sD.getPosition();
        // Calcolo il vettore dal nemico al sottomarino
        const Vector3 vectorToSub = sDCoord - position;
        const float sDToEnemyDistance = vectorToSub.modulo();
        ...
        const float distanceToTravel =
                fminf(speed * Physics::stepSeconds, sDToEnemyDistance);
    ...
    if (rotatesTowardsPlayer) {
        const Vector3 vectorToSub = sD.getPosition() - position;
