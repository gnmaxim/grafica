...
Point3 SecretTech::getPosition(const float gameSeconds) const {
    const float lifeTime = gameSeconds - spawnTime;
    return initialPosition + -Vector3::GetUpVector() * fallSpeed * lifeTime;
}

void SecretTech::render(const float gameSeconds, bool useShadow) const {
	const Point3 currentPosition = getPosition(gameSeconds);
	static constexpr float junkScale = 3.f;
    glPushMatrix();
    {
